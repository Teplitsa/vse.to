...
class Form_Frontend_Event extends Form_Frontend
{
    /**
    ...
    public function init()
    {
        ...
        $this->add_component($tab);
            ...
            $fieldset = new Form_Fieldset('main_props', array('label' => 'Основные свойства (неизменные параметры события)'));
            $cols->add_component($fieldset);
            
            // ----- Caption
            $element = new Form_Element_Input('caption',
                array('label' => 'Название', 'required' => TRUE,'disabled' => TRUE),
                array('maxlength' => 255)
            );
            $element
            ...
            $element = new Form_Element_Select('town',
                $towns,
                array('label' => 'Город','disabled' => TRUE)
            );
            $fieldset->add_component($element); 
            ...
            $element = new Form_Element_Select('access',
                ...
                array('label' => 'Формат события','disabled' => TRUE)
            );
            $fieldset->add_component($element);
        ...
        $fieldset = new Form_Fieldset('product_sections', array('label' => 'Основная категория анонса'));
        ...
        {        
            ...
            foreach ($sectiongroups as $sectiongroup)
            {
                ...
                else
                {
                    ...
                    $history = URL::uri_to('frontend/catalog/products/control', array('action' => 'sections_select'), TRUE);

                    $sections_select_url = URL::to('frontend/catalog/sections/control', array(
                                                        'action' => 'select',
                ...
                {
                    $options[$plist->id] = $plist->caption;
                }
            ...
            $this->add_component($tab);
                ...
                // ----- Description
                $tab->add_component(new Form_Element_Wysiwyg('description', array('label' => 'Описание')));

            // ----- Address tab
            $tab = new Form_Fieldset_Tab('address_tab',array('label' => 'Адрес'));
    ...
    public function render_js()
    {
        ...
        $properties_url = URL::to('frontend/catalog/products/control', array(
        ...
        $on_sections_select_url = URL::to('frontend/catalog/products/control', array(
        ...
        jQuery::add_scripts();
        Layout::instance()->add_script(Modules::uri('catalog') . '/public/js/frontend/product_form.js');
    }
}
