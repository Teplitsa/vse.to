...
/**
 * A simple datetime selection input
 *
 * @package    Eresus
 * @author     Skryabin Ivan (skr@mail.mipt.ru)
 */
class Form_Element_DateTimeSimple extends Form_Element_Input
{
    /**
    ...
    {
        return Kohana::config('datetime.datetime_format');
    }
    ...
    {
        static $datetimesimple_scripts_installed = FALSE;
        
        if ( ! $datetimesimple_scripts_installed)
        {
            if (Modules::registered('jquery')) {
                jQuery::add_scripts();
                Layout::instance()->add_script(Modules::uri('jquery') . '/public/js/datetimesimple.js');
            }
            $datetimesimple_scripts_installed = TRUE;
        }
    }
