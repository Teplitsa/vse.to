...
$('#'+settings['prefix']+id).html('&nbsp Новое фото ');
$('<div />').css({'background-image': ('url('+e.target.result+')'), 'background-repeat': 'no-repeat','margin-top': '10px', 'background-size': settings['scale'] }).addClass(settings['prefix']+'thumb').appendTo($('#'+settings['prefix']+id));
};
