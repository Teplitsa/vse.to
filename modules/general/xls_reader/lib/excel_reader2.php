...
class Spreadsheet_Excel_Reader {
	...
	function _format_value($format,$num,$f, $debug = FALSE) {
		// 49==TEXT format
        ...
        // strip out invalid "zero" bytes
        $format = preg_replace('/\x00/', '', $format);

        // "00000" - zero-padded formats
        if (preg_match('/^0+$/', $format))
        {
            $formatted = sprintf("%0" . strlen($format) . "s", $num);
            return array('string'=>$formatted, 'formatColor'=>null);
        }
