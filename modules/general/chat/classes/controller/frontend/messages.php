...
class Controller_Frontend_Messages extends Controller_FrontendCRUD
{
    /**
     * Setup actions
     * 
    ...
    {        
        ...
        $this->_form  = 'Form_Frontend_Message';
    ...
    public function widget_messages()
    {
        ...
        $view = new View('frontend/messages');
        ...
        if ($dialog_id > 0)
        {
            ...
            $form = new Form_Frontend_SmallMsg($model);
    ...
    public function add_breadcrumbs(array $request_params = array())
    {
        ...
        $dialog = Model::fly('Model_Dialog')->find((int) $request_params['dialog_id']);
       
        if (! $dialog->id)
            return;

        Breadcrumbs::append(array(
            'uri' => URL::uri_to('frontend/messages', array(
                'dialog_id' => $dialog->id,
                'history' => $request_params['history']),TRUE),
            'caption' => $dialog->opponent->name
        ));
    }        
}
