...
class Base_Calendar {
         ...
         * @see Calendar, Calendar::setLang()
         */
        public static $DOWS = array(
                        'en' => array('Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'),
                        'ru' => array('Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб')
                );

	/**
	 * Шаблон генерации ссылок для всех дат.
	 * @var string
	 * @access private
	 */
	...
	/**
	 * Ключ замены по шаблону
	 * @var string
	 * @access private
	 */
	var $patternSearch = NULL;
	 ...
	 */
	function __construct($_lng = 'en', $_options = false, $_cssPrefix = 'cld_') {
		if(!isset(self::$DOWS[$_lng])) {
			trigger_error("Undefined language was specified.", E_USER_WARNING);
			return false;
		}
		$this->dows = self::$DOWS[$_lng];
		$this->setLang($_lng);
	...
	function setLang($_lng) {
		if(isset(self::$MONTHES[$_lng]) && isset(self::$DOWS[$_lng])) {
			$this->lng = $_lng;
			return true;
		} else {
	...
	/**
	 * Добавляет гиперссылку для всех дат по стандартному шаблону.
         * Все предыдущие ссылки переопределяются
	 * @access public
	 * @param string $_url Ссылка
	 * @see addDateLink(), removeUDateLink(), removeDateLink(), 
	 * removeDateLinks()
	 */
	function addDateLinks($_url,$search) {
                $this->patternLink = $_url;
                $this->patternSearch = $search;
		return true;
	}
	...
	function genUMonth($_date, $_mark = false) {
		...
		$monthes = (isset(self::$MONTHES[$this->lng]))?
			self::$MONTHES[$this->lng]:date('F', $_date);
		
		$dows = self::$DOWS[$this->lng];
		...
		for($i = 1, $striper = $wDay % 2; $i <= date('t', $_date); 
			...
			$d = mktime(0, 0, 0, $month, $i, $year);
                        ...
                        if ($this->patternLink !== NULL) {
                            $this->dateLinks[$d] = str_replace($this->patternSearch, $d, $this->patternLink);                          
                        }
