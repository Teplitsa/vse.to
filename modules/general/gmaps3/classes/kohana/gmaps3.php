...
//     $js .= "google.maps.event.addListener({$k}_{$this->id}, 'click', function() {
//                    window.location = this.title; // in this window
//                });";
     //$js .= "}, ". $i." * 200);";
	 }
  ...
  /**
    ...
    $url .= $this->config->language == 'i18n' ? substr(I18n::lang(), 0, 2) : $this->config->language;
		...
		// Set params
    if (!empty($address))
		  $url .= '&address='.htmlentities($address,ENT_QUOTES,"UTF-8");
    ...
    { 
                    if ($decode) {
                        return is_array($response->results)?current($response->results):$response->results;
                    } else {
